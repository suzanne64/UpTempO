...
def processICE(strdate='default',nors='n'):
    #strdate= yyyymmdd
    ...
    path=f'/Volumes/GoogleDrive/My Drive/UpTempO/Satellite_Fields/NSIDC_ICE/'
    ...
    if nors == 'n':
        ...
        ftp.cwd('/pub/DATASETS/nsidc0081_nrt_nasateam_seaice/south/')

    noFile = True
    numDaysBack = 0
    while noFile:
        print('numDaysBack',numDaysBack)
        icefile = f'{path}{reg}{strdate[:4]}/{servfile}'
        print('line 46 ice',servfile)
        if numDaysBack == 7:
            ftp.quit()
            print(servfile+' was not found on the nsidc server.')
            break
        try:
            localfile=open(icefile,'wb')
    ...
    print('line 72 in pfields',regpath)
    cadd='f18'
    path=f'/Volumes/GoogleDrive/My Drive/UpTempO/Satellite_Fields/NSIDC_ICE/{regpath}/nt_{strdate}_{cadd}_nrt_{nors}.bin'
    print('line 75 in pfields',path)

    data=np.fromfile(path,dtype='ubyte')
    print('line 79 in pfields')
    data=data[300:]
    data=data/255.0
    ...
    sh=np.shape(data)
    print('line 86 in pfields',sh)
    ice=np.zeros(sh)
    for i in range(sh[0]):
        ice[i,:]=data[sh[0]-1-i,:]
        
    # establish x/y grid
    print('line 94 in pfields')
    print(f'/Volumes/GoogleDrive/My Drive/UpTempO/Satellite_Fields/NSIDC_ICE/{regpath}/ICExx.dat')
    
    if not os.path.exists(f'/Volumes/GoogleDrive/My Drive/UpTempO/Satellite_Fields/NSIDC_ICE/{regpath}/ICExx.dat'):
        # probably not the best way
        print('pfields line 94')
        dx = dy = 25000
        x = np.arange(-3850000, +3750000, +dx)
        y = np.arange(+5850000, -5350000, -dy)
        xx, yy = np.meshgrid(x,y)
        np.savetxt('/Volumes/GoogleDrive/My Drive/UpTempO/Satellite_Fields/NSIDC_ICE/'+regpath+'/ICExx.txt',xx,fmt="%.2f")
        np.savetxt('/Volumes/GoogleDrive/My Drive/UpTempO/Satellite_Fields/NSIDC_ICE/'+regpath+'/ICEyy.txt',yy,fmt="%.2f")
    ...
    np.savetxt('/Volumes/GoogleDrive/My Drive/UpTempO/Satellite_Fields/NSIDC_ICE/'+regpath+'/ICE-'+strdate+'.txt',ice,fmt="%.2f")
    return strdate, ice, xx, yy
