...
def TimeSeriesPlots(bid,quan='Temp'):
    ...
    dateobjs=[dt.datetime(years[i],months[i],days[i],hours[i]) for i in range(n)]
    ...
    if quan == 'Salinity':
        if 'sdepths' in binf:
            depths=binf['sdepths']
            ind1=[header.index(item) for item in header if item.startswith('S') and not item.startswith('SUB')]
        if 'CTDSs' in binf:
            depths=binf['CTDSs']
            ind1=[header.index(item) for item in header if item.startswith('CTD-S')]
        yaxlab='Salinity'
        ax.set_ylim(20,40)
        # if bid == '300234061160500': ax.set_ylim(-2.0,2.0)
        # else: ax.set_ylim(-2.0,10.0)
...
def OverviewMap(strdate=None):
    ...
    for c in curbuoys:
        ...
        if strdate != None:
            ...
            for d in data:
                sd=d.split(' ')
                if (sd[0] == objdate.year) and (sd[1] == objdate.month) and (sd[2] == objdate.day):
                    last=d
                    foundlast=1
                    break
